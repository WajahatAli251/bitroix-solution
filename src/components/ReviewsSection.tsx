...
// Pre-optimized external images with smaller sizes
const REVIEW_IMAGES = [
  "https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=400&q=60&auto=format&fit=crop",
  "https://images.unsplash.com/photo-1518770660439-4636190af475?w=400&q=60&auto=format&fit=crop",
  "https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?w=400&q=60&auto=format&fit=crop",
  "https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=400&q=60&auto=format&fit=crop",
  "https://images.unsplash.com/photo-1579684385127-1ef15d508118?w=400&q=60&auto=format&fit=crop",
...
const reviews = [
  ...
  },
  {
    ...
    rating: 5,
    avatar: "👩‍💼",
    image: REVIEW_IMAGES[5]
  }
];

// Memoized review card for better performance
const ReviewCard = memo(({ review, isDuplicate = false }: { review: typeof reviews[0]; isDuplicate?: boolean }) => (
  <article 
    className="relative p-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group min-h-[280px]" 
    itemScope={!isDuplicate ? true : undefined}
    itemType={!isDuplicate ? "https://schema.org/Review" : undefined}
  >
    {/* Background Image with lazy loading and optimized size */}
    <img 
      src={review.image}
    ...
    <div className="relative z-10">
      <div className="mb-4">
        <h4 className="text-lg font-semibold text-white drop-shadow-lg" itemProp={!isDuplicate ? "author" : undefined}>
          {review.name}
        </h4>
      ...
      <p className="text-gray-200 leading-relaxed italic drop-shadow-md text-sm" itemProp={!isDuplicate ? "reviewBody" : undefined}>
        "{review.comment}"
      </p>
    </div>
  </article>
          ...
          <div className="embla__container flex">
            ...
            {reviews.map((review, index) => (
              <div key={`duplicate-${index}`} className="embla__slide flex-none w-80 md:w-96 mr-6">
                <ReviewCard review={review} isDuplicate />
              </div>
            ))}
          </div>
        </div>
